...
#line 2 "user\\Robocon_CAN.c"

CanTxMsg TxMessage;
CanRxMsg RxMessage;
CanRxMsg CAN_Queue[256];	
s32 CAN_QueueCounter = 0;					
u8 CAN_QueueHead = 0;
u8 CAN_QueueTail = 0;
...
void CAN_Configuration(void)
{
	...
	CAN_InitStructure.CAN_SJW = ((uint8_t)0x00);
	CAN_InitStructure.CAN_BS1 = ((uint8_t)0x02);
	CAN_InitStructure.CAN_BS2 = ((uint8_t)0x01);
...
void Main_TX(void)
{
	...
	TX_MailBox = CAN_Transmit(((CAN_TypeDef *) (((uint32_t)0x40000000) + 0x6400)), &TxMessage);
...
void CAN_addToQueue(CanRxMsg RxMsg)
{										   	
	CAN_Queue[CAN_QueueTail] = RxMsg;			
	CAN_QueueTail++;							
	if(CAN_QueueTail == 256)		
		CAN_QueueTail = 0;
	CAN_QueueCounter++;
}
