...
//need initial values
float max_acc= 0.5;  			//10
float max_pwm= 200;  //max speed
float min_ac= 0;
...
void pos_cal_function_mode(void)
{
	...
	{	
		...
		else if (predictor>count_dest/2 && predictor<count_dest)
	...
	else 
	{
	    if (abs(predictor) == 0)
			...
			count_input=sqrt_rt1+offset;
		       ...
		       && abs(predictor) <= abs(count_dest-(max_pwm*max_pwm/(2*max_acc))))
			count_input=max_speed;

		else if  ( abs(predictor) > abs(count_dest-(max_pwm*max_pwm/(2*max_acc))) 
