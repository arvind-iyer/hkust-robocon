...
static u8 rx_successful_data[BLUETOOTH_DATA_LENGTH] = {0};
...
/***
	...
	rx_data[1] 			= data_length;			//	0 - 8
	rx_data[2..n-3] = data[x[n]]	// the data
...
BLUETOOTH_COM_IRQHandler
{
	...
	if (USART_GetITStatus(BLUETOOTH_USART, USART_IT_RXNE) != RESET) {
		...
		switch (rx_state) {
			...
			case 1:
				// STATE [1]: Data length
				if (rx_data >= 1 && rx_data <= BLUETOOTH_DATA_LENGTH) {
					++rx_state;
			...
			default:
				if (rx_state < BLUETOOTH_PACKAGE_LENGTH) {
					if (rx_state < rx_data_length + 2) {	// STATE [2..n-3] : Getting the data array
						rx_successful_data[rx_state-2] = rx_data;
						++rx_state;
					} else if (rx_state == rx_data_length + 2) {	// STATE [n-2]: Check byte 1
						u8 check_byte = (rx_successful_data[0] & 0x03) | 
														((rx_successful_data[1] & 0x03) << 2) | 
														((rx_successful_data[2] & 0x03) << 4) | 
														((rx_successful_data[3] & 0x03) << 6);
						if (rx_data == check_byte) {
							++rx_state; 
						} else {
							rx_data_reset();
						}
					} else if (rx_state == rx_data_length + 3) {	// STATE [n-1]: Check byte 2
						u8 check_byte = (rx_successful_data[4] & 0x03) | 
														((rx_successful_data[5] & 0x03) << 2) | 
														((rx_successful_data[6] & 0x03) << 4) | 
														((rx_successful_data[7] & 0x03) << 6);
						if (rx_data == check_byte) {
							++rx_state; 
						} else {
							rx_data_reset();
						}
					} else if (rx_state == rx_data_length + 4) {	// STATE [n]: Sleep command
