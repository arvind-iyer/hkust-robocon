...
static u8 menu_id = 0;	// SELECTED ITEM
...
{
	u8 pos = (tft_orientation % 2 ? MAX_HEIGHT : MAX_WIDTH);
	...
	batt_color = batt <= 114 ? RED : (batt <= 120 ? 0xFCC0 : GREEN);
	...
	while (1) {
		if (ticks_img != get_ticks()) {
			...
			if (ticks_img % 50 == 2) {
				// Check button input
				button_update();
				if (button_pressed(BUTTON_JS1_DOWN) == 1 || button_hold(BUTTON_JS1_DOWN, 15, 5)) {
					// Go down the list
					if (menu_id < menu_count - 1) {
						++menu_id; 
					}
				} else if (button_pressed(BUTTON_JS1_UP) == 1 || button_hold(BUTTON_JS1_UP, 15, 5)) {
					if (menu_id > 0) {
						--menu_id;
					}
				}
			...
			if (ticks_img % 50 == 7) {
				...
				tft_set_text_color(RGB888TO565((500 - Abs(500 - ticks_img)) * 0xFF / 500 * 0x010101));
				tft_prints(3,0,":");
				tft_set_text_color(WHITE);
				...
				// Menu list
				for (u8 i = 0, y = 1; i < menu_count; ++i, ++y) {
					if (i == menu_id) {
					...
					tft_prints(1, y, "%s", menu_list[i].title);
				}
