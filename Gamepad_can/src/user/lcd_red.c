...
u8 x , y;	
	...
	for(y=0;y<10;y++)
		for(x=0;x<16;x++){
			text_color[x][y] = (text_bg_color_prev[x][y] & 0x40 ? RED : 0) | (text_bg_color_prev[x][y] & 0x20 ? GREEN : 0 ) | (text_bg_color_prev[x][y] & 0x10 ? BLUE : 0); //text:red green blue
			bg_color[x][y] = (text_bg_color_prev[x][y] & 0x04 ? RED : 0) | (text_bg_color_prev[x][y] & 0x02 ? GREEN : 0) | (text_bg_color_prev[x][y] & 0x01 ? BLUE : 0); //text:red green blue
		}
}
/**
...
void tft_prints(u8 x, u8 y, const u8 * pstr, ...)
{
	...
	while (*fp)	{
		...
		//	text_bg_color_prev[x][y] = (bg_color[x][y] & 0x8000 ? 0x4 : 0) & (bg_color[x][y] & 0x0400 ? 0x2 : 0) & (bg_color[x][y] & 0x0010 ? 0x1 : 0); //bg :red green blue
			if (x >= CHAR_MAX_X) {
				x = 0;
				y++;
			} else {
