...
static u8 buzzer_volume = 20;	                	    /*!< Volume percentage for the buzzer, 0 - 100 (101 for full buzz) */
...
const TIMER buzzer_timer = {BUZZER_TIM, Ch3};
...
void buzzer_init(void)
{
  ...
  /* GPIO Init */
  BUZZER_GPIO->gpio_init(GPIO_Speed_50MHz, GPIO_Mode_AF, GPIO_OType_PP, GPIO_PuPd_NOPULL);

  GPIO_PinAFConfig(BUZZER_GPIO->gpio, BUZZER_GPIO->get_pin_source(), GPIO_AF_TIM8);

  timer_init(BUZZER_TIM, SystemCoreClock / BUZZER_COUNT_PER_SECOND - 1, TIM_CounterMode_Up, buzzer_note_period, TIM_CKD_DIV2);
  pwm_timer_init(buzzer_timer, TIM_OCMode_PWM1, TIM_OutputState_Enable, buzzer_volume, TIM_OCPolarity_High);
